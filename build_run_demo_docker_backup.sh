...
docker build . -t genetic_car_backup -f backup.Dockerfile
xhost +local:docker
docker run --net=host -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$DISPLAY $CAM0 $CAM1 $CAM2 $CAM3 --rm -it genetic_car_backup
